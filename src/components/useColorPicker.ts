...
export function useColorPicker(props: ColorPickerProps) {
  ...
  const setColor = (_color: string) => {
    const unpack = hex2rgb(_color)
    if (color.value !== _color && unpack) {
      color.value = _color
      rgb.value = unpack
  ...
  const setHSL = (_hsl: number[]) => {
    hsl.value = _hsl
    rgb.value = hsl2rgb(_hsl)
  ...
  onMounted(() => {
    ...
    drawCircle()
    drawMask()
    setColor(props.value || DEFAULT_START_COLOR)
  })
